...
/************THE PROGRAM HAS BE ADAPTED TO FUNCTION WITH OPENCV, PREVIOUSLY IT USED THE PNG LIBRARY. THE DISPARITY MAP INPUT HAS
ALSO BEEN ADAPTED, INSTEAD OF COMPUTING A SEMI-DENSE DISPARITY MAP, A DENSE DISPARITY MAP IS COMPUTED PREVIOUSLY BY OPENCV SGBM 
AND USED AS AN INPUT FOR THIS PROGRAM*************/
...
void SPSStereo::setInputData(const Mat & leftImage, const Mat & dispInput) {
	setLabImage(leftImage);
	computeInitialDisparityImage(leftImage, dispInput);
}

void SPSStereo::setLabImage(const Mat & leftImage) {
	...
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			//Mat pix = leftImage.get_pixel(x, y);
			double pix_r = leftImage.at<Vec3b>(x, y)[2];
			double pix_g = leftImage.at<Vec3b>(x, y)[1];
			double pix_b = leftImage.at<Vec3b>(x, y)[0];
			float correctedR = sRGBGammaCorrections[pix_r];
			float correctedG = sRGBGammaCorrections[pix_g];
			float correctedB = sRGBGammaCorrections[pix_b];
...
void SPSStereo::computeInitialDisparityImage(const Mat & leftImage, const Mat & dispInput) {
	SGMStereo sgm;
	sgm.compute(leftImage, dispInput, initialDisparityImage_);
}
...
void SPSStereo::makeOutputImage( Mat & segmentImage, Mat & segmentDisparityImage) const {
 	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			int pixelSegmentIndex = labelImage_[width_*y + x];
			segmentImage.at<uchar>(x, y) = pixelSegmentIndex;
			double estimatedDisparity = segments_[pixelSegmentIndex].estimatedDisparity(x, y);
			if (estimatedDisparity <= 0.0 || estimatedDisparity > 255.0) {
				segmentDisparityImage.at<uchar>(x, y) = 0;
			} else {
				//unsigned short depth = static_cast<unsigned short>(estimatedDisparity*outputDisparityFactor_ + 0.5);
				segmentDisparityImage.at<uchar>(x, y) = estimatedDisparity;
				//segmentDisparityImage.at<uchar>(x, y, 1) = y * depth/ fs;
				//segmentDisparityImage.at<uchar>(x, y, 0) = x * depth/ fs;
				//The X Y Z in the 3D world have been stored in a 3D array where they are stored in 0 1 2 respectively for each x y pixel
			}
		}
	}
}
