...
private:
	...
	void allocateBuffer();
	void freeBuffer();
	void setInputData(const Mat & leftImage, const Mat & disparityInput);
	void setLabImage(const Mat  & leftImage);
	void computeInitialDisparityImage(const Mat & leftImage, const Mat & dispInput);
	...
	void makeOutputImage(Mat & segmentImage, Mat & segmentDisparityImage) const;
