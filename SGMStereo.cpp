...
void SGMStereo::compute(const Mat & leftImage,
						const Mat & dispInput,
						float* disparityImage)
{
	...
	setImageSize(leftImage, dispInput);

        // Instead of performing semi dense SGM Stereo matching, The disparity image for SPS smoothing is set to the output of the OpenCV
        // SGBM algorithm.
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			disparityImage[width_*y + x] = dispInput.at<short>(x, y);
		}
	}
	/*freeDataBuffer();
...
void SGMStereo::setImageSize(const Mat & leftImage, const Mat & dispInput) {
	width_ = static_cast<int>(leftImage.rows);
	height_ = static_cast<int>(leftImage.cols);
	if (dispInput.rows != width_ || dispInput.cols != height_) {
...
void SGMStereo::computeCostImage(const Mat & leftImage, const Mat & rightImage) {
...
void SGMStereo::convertToGrayscale(const Mat & leftImage,
								   ...
								   unsigned char* rightGrayscaleImage) const
{
	for (int y = 0; y < height_; ++y) {
		for (int x = 0; x < width_; ++x) {
			...
			//double pix_rd = pix_r, pix_gd = pix_g, pix_bd = pix_b;
			leftGrayscaleImage[width_*y + x] = static_cast<unsigned char>(0.299*pix_r + 0.587*pix_g + 0.114*pix_b + 0.5);
			//pix = rightImage.get_pixel(x, y);
			//Vec3b pix = rightImage.at<Vec3b>(x, y);
			pix_r = rightImage.at<Vec3b>(x, y)[2];
			pix_g = rightImage.at<Vec3b>(x, y)[1];
			pix_b = rightImage.at<Vec3b>(x, y)[0];
			rightGrayscaleImage[width_*y + x] = static_cast<unsigned char>(0.299*pix_r + 0.587*pix_g + 0.114*pix_b + 0.5);
		}
	}
}
